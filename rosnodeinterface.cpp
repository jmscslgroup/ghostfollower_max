...
// Generated 26-Jun-2021 00:06:48
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"ghostfollower_max\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(ghostfollower_max_M, (NULL));
        ghostfollower_max_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(ghostfollower_max_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(ghostfollower_max_M)
                == (NULL)) || rtmGetStopRequested(ghostfollower_max_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(ghostfollower_max_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
